...
<head>
    ...
    <title>Dashboard Petugas | Klinik Sehat</title>
...
<div class="max-w-6xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-10 bg-white p-8 rounded-[2rem] shadow-sm border border-slate-200 gap-6">
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-slate-800">Halo, <span id="nama-petugas"></span>!</h1>
                <p class="text-slate-500">Selamat bertugas di <span id="role-petugas" class="text-emerald-600 font-bold uppercase tracking-wider text-sm"></span> Panel</p>
            </div>
        </div>
        <button onclick="logout()" class="w-full md:w-auto bg-red-500 hover:bg-red-600 text-white px-8 py-3 rounded-2xl font-bold transition-all shadow-lg shadow-red-100 flex items-center justify-center gap-2">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <h3 id="total-antrian" class="text-3xl font-black text-slate-800">-</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <p class="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Sudah Dilayani</p>
            <h3 id="total-selesai" class="text-3xl font-black text-emerald-500">-</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <p class="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Tanggal</p>
            <h3 id="tgl-hari" class="text-lg font-bold text-slate-700">-</h3>
        </div>
    ...
    <div class="grid md:grid-cols-2 gap-8 mb-10">
        <a href="monitoring.php" class="group bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 hover:shadow-emerald-200/50 transition-all border border-slate-100 hover:-translate-y-2">
            ...
            <div class="mt-8 flex items-center text-emerald-600 font-bold gap-2 group-hover:gap-4 transition-all">
        ...
        <a href="display.php" target="_blank" class="group bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 hover:shadow-blue-200/50 transition-all border border-slate-100 hover:-translate-y-2">
            ...
            <div class="mt-8 flex items-center text-blue-600 font-bold gap-2 group-hover:gap-4 transition-all">
    ...
    <p class="text-center text-slate-400 mt-12 text-sm">&copy; 2026 Digital Clinic System &bull; Universitas Sebelas Maret</p>
</div>
 
<script>
// Set info user
document.getElementById('nama-petugas').textContent = localStorage.getItem('nama') || 'Petugas';
document.getElementById('role-petugas').textContent  = localStorage.getItem('role') || '';
document.getElementById('tgl-hari').textContent = new Date().toLocaleDateString('id-ID', {day:'2-digit', month:'short', year:'numeric'});
...
async function loadStats() {
    try {
        ...
        document.getElementById('total-antrian').textContent = data.total  ?? 0;
        document.getElementById('total-selesai').textContent = data.selesai ?? 0;
    } catch (e) {}
}
...
async function updateChart() {
    try {
        const res    = await fetch('/api/get_statistik_poli.php');
        ...
        myChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Jumlah Pasien',
                    data: result.data,
                    backgroundColor: ['rgba(16,185,129,0.7)','rgba(59,130,246,0.7)','rgba(249,115,22,0.7)','rgba(139,92,246,0.7)'],
                    borderRadius: 12,
                    borderSkipped: false,
                }]
            },
...
loadStats();
updateChart();
setInterval(() => { loadStats(); updateChart(); }, 30000);
</script>
